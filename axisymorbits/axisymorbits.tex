...
% Externalizing figures should probably be disabled when not generating.
\newif\ifextgen%\extgentrue
\ifextgen\tikzexternalize\fi % activate!
...
In the 1D case where $\phi$ depends only on $z$, there are two exact
constants of the motion which are the total energy $W$ and the
magnetic moment, which in the present uniform magnetic field can be
taken as the perpendicular kinetic energy $W_\perp$. The perpendicular
motion is then entirely decoupled from the parallel and can be
ignored. However, when $\phi$ varies with radius $r$, and a transverse
electric field $E_r$ exists, the $W_\perp$ (magnetic moment)
invariance is broken, and the only strict invariant in addition to $W$
is the canonical angular momentum about the $z$-axis:
$p_\theta=r(v_\theta-\Omega r/2)$, where $\Omega$ is the
gyro-frequency. The effect of $p_\theta$ conservation is mostly to
restrict the range of variation of the orbit's radius to what in the
probe literature are called ``magnetic bottles''
(e.g. \cite{Laframboise1993}). At radii greater than approximately the
gyro-radius $\rho=v_\perp/\Omega$, conservation of $p_\theta$
contributes little to parallel particle dynamics, serving mostly to
localize the orbit in radial position, within approximately one
gyro-radius. Fig.\ \ref{fig:orbitview} illustrates the kind of orbit
that results.
\begin{figure}
\ifextgen
...
\begin{figure}[htp]
  \centering
  \includegraphics[width=0.6\hsize]{detraporbit1}
...
\begin{figure}%[htp]
  \centering
  \includegraphics[width=0.5\hsize]{EnvW2}
...
\begin{equation}
  \label{eq:adhocadj}
  m=\mathtt{nint}(n/2+\mathtt{max}(n/2-3.3,0)*0.75)  
\end{equation}
(instead of
...
%Unlike the previous illustration in Fig.\ \ref{fig:islands} t
...
\begin{figure}
\ifextgen
  \tikzsetnextfilename{Figure6}
    \begin{tikzpicture}\node (image)
  {\noindent
  \includegraphics[width=0.491\hsize]{phas905}\hskip-2em (a)\hskip1em 
  \includegraphics[width=0.488\hsize]{poin905}\hskip-2em (b)\phantom{M}}
;\end{tikzpicture}
\else
...
\ifextgen
  ...
  \includegraphics[width=0.488\hsize]{poin940}\hskip-2em (b)\phantom{M}}
  ;\end{tikzpicture}
\else
...
\ifextgen
  ...
  \includegraphics[width=0.488\hsize]{poin9100}\hskip-2em (b)\phantom{M}}
  ;\end{tikzpicture}
\else
\includegraphics[width=\hsize]{Figure8}
\fi
  \caption{(a) Analytic energy trajectories for different harmonics
    ($n$), and fixed magnetic field strength and (b) Poincar\'e plot of
...
\begin{figure}
\ifextgen
  \tikzsetnextfilename{Figure9}
  \begin{tikzpicture}\node[anchor=south west] (image) 
  {\noindent
  \includegraphics[width=0.491\hsize]{phas610}\hskip-2em (a)\hskip1em 
  \includegraphics[width=0.488\hsize]{poin610}\hskip-2em (b)\phantom{M}};
 \node[anchor=north west] (image)
  {\includegraphics[width=0.491\hsize]{phas640}\hskip-2em (c)\hskip1em 
  \includegraphics[width=0.488\hsize]{poin640}\hskip-2em (d)\phantom{M}}
  ;\end{tikzpicture}
\else
\includegraphics[width=\hsize]{Figure9}
\fi
...
\ifextgen
  ...
  {\includegraphics[width=0.491\hsize]{phas18200}\hskip-2em (a)\hskip1em 
   \includegraphics[width=0.488\hsize]{poin18200}\hskip-2em (b)\phantom{M}}
  ;\end{tikzpicture}  
\else
\includegraphics[width=\hsize]{Figure10}
\fi
...
\begin{align}
  ...
  &=
   {n(\sqrt{\wp}-\sqrt{\wr })\over\sqrt{2} \sqrt{w+\wp}}
   \left[{\sqrt{\wp}\,n/2\over (1-\sqrt{\wp})^m} + 
     {\pi\over8}\right].\nonumber
\end{align}
...
\left[  {E_{r0}\over
 ...
 \left[{\sqrt{\wr }\,n/2\over (1-\sqrt{\wr })^m} + 
     {\pi\over8}\right]^{-1/2}.\nonumber
\end{align}
...
&\sqrt{\wrn} \pm\left[  {E_{r0}v_\perp\over
 ...
 \left[{\sqrt{\wrn }\,n/2\over (1-\sqrt{\wrn })^m} + 
     {\pi\over8}\right]^{-1/2}.\nonumber 
\end{align}
It is in this equation that one must use the adjustment of $m$ of eq.\
(\ref{eq:adhocadj}) for high harmonics. Overlap occurs between the $n$
and $n+2$ harmonic islands when
...
\begin{figure}
\ifextgen
  \tikzsetnextfilename{Figure11}
  \begin{tikzpicture}\node (image)
  {\noindent
  \includegraphics[width=0.52\hsize]{trapdomain}\hskip-2em(a)\hskip1em
  \includegraphics[width=0.46\hsize]{tdlineouts}\hskip-2em(b)\phantom{M}}
  ;\end{tikzpicture}
\else
\includegraphics[width=\hsize]{Figure11}
\fi
...
\begin{figure}
\ifextgen
  \tikzsetnextfilename{Figure12}
  \begin{tikzpicture}\node (image)
  {\noindent
  \includegraphics[width=0.48\hsize]{vspace05}\hskip-2em (a)\hskip1em
  \includegraphics[width=0.48\hsize]{vspace12}\hskip-2em (b)\phantom{M}}
  ;\end{tikzpicture}
\else
\includegraphics[width=\hsize]{Figure12}
\fi
...
\begin{align}
  \label{eq:integrals}
  F_n&={n^2\over 2\sqrt{2}}g_m+{n\pi\over16\sqrt2} g_0,\\
\noalign{with}
...
%%% TeX-master: t
%%% End:
% 9060 words from start of section 1 
% including 36 equations of length maybe 35 words ~1000 words.
